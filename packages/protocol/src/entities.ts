...
export const ExternalAppInfo = z.object({
  ...
  /**
   * 떠 있는(또는 마지막으로 떴던) 프로세스가 읽은 앱 폴더의 지문 — 이 host에서 한 번도 뜨지 않았으면 없다 (M4 C-4).
   *
   * 열린 화면의 HTML은 그 화면을 열 때 떠 있던 코드의 것이다. 만드는 세션의 턴이 끝나 앱이 새 코드로 다시 뜨면(또는
   * `check`·Restart가 지금 파일로 띄우면) 이 값이 바뀌고, 화면은 그것으로 "내 HTML은 옛 코드다"를 안다. 같은 코드로
   * 다시 뜬 것(죽었다 살아남, 쉬다 깸)은 바뀌지 않는다 — 다시 연다고 달라질 것이 없다. 뜻 없는 열쇠라 대조만 한다.
   */
  codeStamp: z.string().optional(),
  /**
