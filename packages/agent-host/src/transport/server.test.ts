...
describe('서버 회복 경계', () => {
  ...
  it('미래 afterSeq 커서는 이벤트 재전송 대신 resync만 요구한다', async () => {
    ...
    const c = connect(port)
    await c.open()
    c.send({ kind: 'hello', token: TOKEN, protocolVersion: PROTOCOL_VERSION, afterSeq: 9 })
    await c.wait(() => c.frames.some((f) => f.kind === 'hello_ok'))
  ...
  it('허용되지 않은 upgrade 요청은 HTTP 계층에서 거부된다', async () => {
    const { port } = await start(async () => ({ ok: true }), { allowUpgrade: () => false })
...
describe('HTTP seam', () => {
  it('onHttp가 상태/헤더와 request 컨텍스트를 제어한다', async () => {
    const { port } = await start(async () => ({ ok: true }), {
      onHttp: (path, request) =>
        path === '/status'
          ? { status: 201, body: request.headers.host ?? 'missing', contentType: 'text/plain', headers: { 'x-centralu-test': 'ok' } }
          : null,
    })
    const response = await new Promise<{ readonly status: number | undefined; readonly body: string; readonly header: string | string[] | undefined }>((resolve, reject) => {
      http.get({ host: '127.0.0.1', port, path: '/status', headers: { Host: `127.0.0.1:${port}` } }, (res) => {
        let body = ''
        res.on('data', (chunk: Buffer) => { body += chunk.toString('utf8') })
        res.on('end', () => resolve({ status: res.statusCode, body, header: res.headers['x-centralu-test'] }))
      }).on('error', reject)
    })
    expect(response).toMatchObject({ status: 201, header: 'ok' })
    expect(response.body).toContain(`127.0.0.1:${port}`)
  })
})
