 ...
 * 없다: 프로젝트 앱의 만드는 세션은 cwd가 프로젝트 뿌리라 앱 폴더의 안내를 스스로 찾아 읽지 않고, 사용자 폴더
 * 앱의 세션은 신뢰할 프로젝트가 없어 폴더의 설정 파일을 읽지 않는다(결정 3 — 가져온 앱의 CLAUDE.md가 만드는
 * 세션에 지시하는 길을 열지 않는다). 규칙의 핵심은 여기 싣고, 자세한 것은 파일을 가리킨다.
 *
