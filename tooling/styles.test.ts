...
describe('ui는 OS를 모른다', () => {
  ...
  /**
   * 자판 이름을 화면에 직접 쓰지 않는다 (이슈 #32).
   *
   * OS 분기 금지의 짝이다. `⌘`는 분기처럼 보이지 않아서 위 검사를 그냥 통과했는데,
   * 결과는 같았다 — **없는 키를 누르라고 말하는 화면**. 게다가 조용히 번진다: `⌘`는
   * 그냥 글자라, 새 힌트를 쓸 때 옆줄을 따라 적으면 그걸로 끝이다. 그렇게 열 파일
   * 열아홉 자리가 됐다.
   *
   * 한 번 쓸어낸 걸로는 안 끝난다. **절반만 쓸린 저장소가 둘 중 어느 쪽보다도 나쁘다** —
   * 어떤 자리는 `⌘`, 어떤 자리는 `Ctrl`이면 화면이 자기 말을 스스로 뒤집는다.
   * 그래서 다시 들어오면 여기서 걸린다.
   *
   * 주석은 뺀다. 주석에서 `⌘A`는 이 코드를 읽는 사람에게 하는 말이지 화면이 하는 말이
   * 아니고, 실제로 뷰어 복사 로직의 근거가 그 표기로 적혀 있다. 검사 대상은 **그려지는
   * 것**이다. `⇧`도 뺀다 — 저 자판들에도 그대로 찍혀 있어서 옮길 말이 없다.
   */
  it('자판 이름은 포트에서 온다 (⌘·⌥를 직접 쓰지 않는다)', () => {
    // 테스트 파일은 맥 표기를 **고정하는** 쪽이라 검사 밖이다 (packages/ui/src/app/shortcut.test.ts)
    const shipped = files.filter((f) => !/\.test\.tsx?$/.test(f))
    const offenders = shipped.filter((f) => {
      const text = readFileSync(join(src, f), 'utf8')
        // `/* */`(JSX 주석 `{/* */}` 포함)와 `//` 줄을 걷어낸 뒤 남는 것이 화면에 가는 것이다
        .replace(/\/\*[\s\S]*?\*\//g, '')
        .replace(/\/\/.*$/gm, '')
      return /[⌘⌥]/.test(text)
    })
    expect(offenders, '<Kbd mod /> · <Kbd alt /> 또는 useShortcut()을 써라').toEqual([])
  })
})
