...
- [x] 대화 기록이 **로컬에만** 저장된다는 사실 명시 (`~/.centralu/store.db`) — README 머리말
...
- [x] **실제 발행 완료** (2026-08-19) — `centralu@0.1.0-beta.1`, `centralu-darwin-arm64@0.1.0-beta.1`.
      레지스트리에서 받아 실행까지 확인했다: 격리 딱지 없음 · 서명 유효 · 실행 비트 정상 ·
      기동 배너가 `build d791d04`(**-dirty 없음**)로 찍힘 = 발행한 커밋 그대로 빌드된 바이너리다.
      두 번 막혔던 것과 그 이유는 아래에 남긴다.
...
### 2-6. 발행에서 두 번 막혔다 (둘 다 npm이 옳았다)

| 막힌 것 | npm이 요구한 것 | 왜 옳은가 |
|---|---|---|
| `--tag` 없는 프리릴리스 | `--tag beta` | 안 그러면 `0.1.0-beta.1`이 `latest`가 되어, 아무 생각 없이 설치한 사람이 베타를 받는다 |
| 2FA 없는 발행 | 계정 2FA 또는 bypass 토큰 | 계정이 털리면 악성 패키지가 그대로 나간다 |

둘 다 **아무것도 발행되지 않은 채** 멈췄다 — 부분 발행 같은 어정쩡한 상태는 없었다.

그리고 2FA를 켠 뒤에도 한 번 더 걸렸다: 이 계정은 **패스키(Touch ID)** 로 등록돼 있어
6자리 코드가 아니라 **브라우저 승인**을 쓴다. 그 대기는 대화형이라 TTY가 아닌 자리에서는
받을 수 없다(npm이 URL만 찍고 EOTP로 종료한다). 자동화된 환경에서 발행하려면
인증 앱(TOTP)을 추가로 등록해 `--otp=`를 쓰거나, 사람이 직접 터미널에서 돌려야 한다.

- [ ] 첫 정식 릴리스 때 `--also-latest`를 **빼는 것**을 잊지 말 것 (베타가 정식을 덮는다)
...
- [ ] 버그 신고 창구 + **`~/.centralu/host.log`를 첨부해달라고 안내**
