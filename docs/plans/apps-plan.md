...
- [x] **E** 건네기 (가벼운 것) — 비밀 값 화면과 RPC, E-3 가져오기와 사람의 확인(꺼진 채 들어오고, `server`·`uses`가 바뀌면 다시 묻는다), E-1 사용자 폴더 앱의 스냅샷(최근 5벌)과 되돌리기·프로젝트 앱은 그 폴더의 git 커밋을 읽기만, E-4 앱 링크(macOS, 딥링크 플러그인 없이 OS의 열기 이벤트) — `m4/share`. E-2 내보내기는 결정 9의 범위 밖이다. 앱 링크의 끝에서 끝은 빌드한 앱에서 손으로 확인해야 한다(`tauri dev`는 스킴을 받지 못한다)
