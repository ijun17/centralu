...
- 매니페스트 필드: `manifestVersion`, `id`, `name`, `version`, `description`, `server { command, args }`, `home`(고정 화면을 여는 도구 이름), `uses { agent?, apps?, host? }`, `secrets?`(이름만), `csp?`. `manifestVersion`은 팀원이 서로 다른 Centralu 버전을 쓸 때를 위한 것이다.
...
두 자리 모두 화면은 도구 호출에서 태어나므로 규격을 벗어나지 않는다. 대화 안 화면에는 우리 사정 하나가 붙는다: 대화 목록은 가상 스크롤이라 화면 밖으로 나간 항목은 DOM에서 사라진다. iframe도 같이 사라지므로, 벗어나기 전에 규격의 teardown 요청을 보내고 정지 이미지를 남긴다. 살아 있는 대화 안 화면은 최근 몇 개로 제한한다.
...
- **앱 화면은 불투명 출처로 격리한다.** 안쪽 iframe에 `allow-same-origin`을 주지 않는다. 그러면 앱끼리 브라우저 저장소가 섞이지 않는다(참조 구현은 모든 서버에 출처 하나를 쓴다). `*.localhost`로 앱마다 출처를 나누는 방법은 macOS 15 이하에서 이름이 풀리지 않아 쓸 수 없다. 대가는 화면이 브라우저 저장소를 못 쓰는 것이다. 상태는 서버에 두는 것이 이 설계의 원칙이라 받아들이고, 저장소를 쓰는 공개 앱이 얼마나 되는지는 스파이크에서 잰다(S-8).
...
- **A-3 프로세스 수명** 처음 필요할 때 띄운다. 크래시는 지수 백오프로 되살리고 세 번 연속 실패하면 멈춘다. 표준에러는 앱별 로그 파일에 남긴다. **쉬는 앱은 내린다**: 열린 화면도 진행 중인 호출도 없으면 5분 뒤 끝낸다.
...
- **A-7 기존 MCP 서버 승인 흡수** 승인한 MCP 서버를 "화면 없는 앱"으로 옮긴다(질문 8). 호출이 호스트를 거치게 되어 기록되고, 오케스트레이터 밖의 세션에도 붙일 수 있고, 목록에서 지울 수 있다. 등록부가 둘이면 승인 흐름도, 붙이는 길도 둘이 된다.
- **A-8 내장 앱과 한 목록** `control`은 컴파일된 내장 앱으로 남는다. 레지스트리가 내장과 외부를 같은 목록으로 보여 준다.

## B. 화면 (UI)

- **B-1 대화 안 화면** 도구 호출 카드 아래에 띄운다. 화면 수명 메시지는 규격대로 보낸다. 가상 스크롤에서 벗어날 때는 teardown을 먼저 보내고 정지 이미지를 남긴다.
...
- [ ] **S** 스파이크 S-1 ~ S-8 (결과는 "스파이크 결과") — S-4·S-5·S-6 통과, S-1·S-2·S-8 진행 중, S-3·S-7은 Codex 로그인 대기
- [ ] **P** 선행 정리 — P-1·P-4·P-5·P-6 완료(#144), P-2·P-3은 A와 함께
- [ ] **A** 런타임 A-1 ~ A-8
- [ ] **B** 화면 B-1 ~ B-7 (대화 안 화면 먼저)
- [ ] **C** 만들기 루프 C-1 ~ C-6 — 도그푸딩 수용 기준
- [ ] **D** 중개 D-1 ~ D-6
- [ ] **E** 건네기 (가벼운 것)
