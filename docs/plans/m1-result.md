...
1. **사용자 훅·플러그인 미로드**: ClaudeAdapter가 `settingSources`를 지정하지 않아 Control Center 세션은 사용자 훅/플러그인 없이 뜬다. 근거: 관제 도구가 사용자의 개인 훅(알림·자동화)을 실행하면 예측 불가능. 되돌리려면 어댑터에서 `settingSources: ['user']` 추가.
...
## G5 실측 (2026-08-15, /Users/ijun17/Desktop/mavis 대상, 에이전트가 대행)
